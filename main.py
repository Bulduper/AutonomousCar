...
frameCuda = jetson.utils.cudaAllocMapped(
    width=1640, height=1232, format='rgb8')
frameCudaRedBgr = jetson.utils.cudaAllocMapped(
    width=640, height=480, format='bgr8')
frameCudaRed = jetson.utils.cudaAllocMapped(
...
def parseEventMsg(channel, data):
    ...
    if 'capture' in data:
        utils.captureImg(images[data['capture']['imgKey']],
                         'signs/'+data['capture']['imgKey'])
...
def detectLane(source):
    global images
    left_line, center_line, right_line = lane_finder_module.getLanePoints(
        source)
    ...
    if center_line.any() and avg_angle:
        ...
        utils.putText(warped, "Avg lane angle: {} deg".format(
            round(avg_angle, 0)))
    ...
    utils.drawStraightLine(warped, (warped.shape[1]//2, warped.shape[0]),
                           (warped.shape[1]//2, warped.shape[0]-30), color=(255, 255, 0))
...
def mainLoop():
    ...
    if SIGN_DETECTION:
        ...
        frameRoi = utils.cropRelative(
            frameCuda, (det_roi_x[0], det_roi_y[0], det_roi_x[1], det_roi_y[1]))
